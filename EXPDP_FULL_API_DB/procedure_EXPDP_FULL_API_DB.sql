...
BEGIN
   ...
   L_INSERT_INTO_LOG  := 'INSERT INTO FULL_EXPDP_LOG (ID, ISTANZA, COMANDO, EVENTO, DESCRIZIONE, TIPO, DATA) VALUES (:1, :2, :3, :4, :5, :6, :7)';
...
-- Check LOG 
TEST_JOB_END := 0;
SELECT COUNT(*) INTO TEST_JOB_END FROM FULL_EXPDP_LOG_EXT WHERE LOG_EXP LIKE '%successfully completed%';
IF TEST_JOB_END = 0
THEN
  SELECT * INTO ABNORMAL_END_JOB FROM FULL_EXPDP_LOG_EXT WHERE LOG_EXP LIKE 'Job%';
  RAISE EXPDP_ORA_ERR;
ELSE
